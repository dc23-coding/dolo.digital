  ...
  <div class="p-4 max-w-md mx-auto">
    ...
    <div class="mb-4 flex justify-center gap-2">
      ...
      <button
        ...
        :class="{ 'bg-blue-500 text-white': units === 'imperial', 'bg-gray-200': units !== 'imperial' }"
      ...
      <button
        @click="units = 'metric'"
        :class="{ 'bg-blue-500 text-white': units === 'metric', 'bg-gray-200': units !== 'metric' }"
        class="px-3 py-1 rounded text-lg"
      >
        Metric
      </button>
    ...
    <div class="mb-4">
      ...
      <select
        ...
        class="w-full p-2 border rounded text-lg"
      >
        <option v-for="type in vehicleTypes" :value="type.name">
          {{ type.name }} ({{ units === 'imperial' ? type.efficiencyImperial : type.efficiencyMetric }} {{ efficiencyLabel() }})
        </option>
    ...
    <!-- Calculate Button -->
      <button
        @click="calculate"
        class="bg-blue-500 text-white px-6 py-3 rounded w-full text-xl font-semibold hover:bg-blue-600 transform transition-transform duration-150 ease-in-out hover:scale-105 active:scale-95"
      >
        Calculate
        </button>
    ...
    <div v-if="results" class="mt-6 p-4 bg-gray-100 rounded">
      ...
      <p class="text-lg">Estimated Fuel Cost: <span class="font-bold">${{ results.fuelCost.toFixed(2) }}</span></p>
      <p v-if="results.earnings !== null" class="text-lg">
        Estimated Earnings: <span class="font-bold">${{ results.earnings.toFixed(2) }}</span>
      </p>
    </div>
  ...
  data() {
    return {
      ...
      vehicleTypes: [
        ...
        { name: 'Semi Truck', efficiencyImperial: 10, efficiencyMetric: 4.25 }, // Average of 8-12 MPG
      ],
    };
  },
  methods: {
    ...
    calculate() {
      ...
      if (this.vehicleType === 'custom') {
        ...
        efficiency = this.units === 'imperial' ? vehicle.efficiencyImperial : vehicle.efficiencyMetric;
      }
